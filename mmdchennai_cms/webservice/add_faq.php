...
if ($operation == 'save_rti') {
    ...
    if ($error != 1) {
        $query = "insert into mmd_faq(question,answer,inserted_by,uploaded_on,updated_on) values('$question','$answer','$sessionId','$currdate','$currdate')";
    ...
    if ($error != 1) {
        $edit_query = "select * from mmd_faq where ques_id = $ques_id";
    ...
    if ($error != 1) {
        $edit_query = "update mmd_faq set question ='$question',answer='$answer', updated_by ='$sessionId' ,updated_on='$currdate'  where ques_id = $ques_id";
        $edit_result = pg_query($db, $edit_query);
        if ($edit_result) {
            $files_arr['status'] = 'ok';
        } else {
    ...
    foreach ($position as $k => $v) {
        $sql =
            'Update mmd_faq SET position_order=' .
            $i .
            ' WHERE ques_id=' .
            $v;
    ...
    if ($error != 1) {
        ...
        $edit_query = "update  mmd_faq set status= '$status' where ques_id = $ques_id ";
        $edit_result = pg_query($db, $edit_query);
        if ($edit_result) {
            $files_arr['status'] = 'ok';
        } else {
