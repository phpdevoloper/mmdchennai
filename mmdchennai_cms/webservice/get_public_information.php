...
if ($status == '') {
    $statuscheck = "select * from $table en order by info_id desc";
    $resultstatus = pg_query($db, $statuscheck);

} else {
    $statuscheck = "select * from $table en where en.status= '$status' order by info_id desc";
    $resultstatus = pg_query($db, $statuscheck);
}
            ...
            <table class="tbl-en-draft  table table-striped table-bordered " width="100%">
                <thead id="thead" style="color:#fff;background:#40c4ff" class="text-center">
                    <tr>
                        ...
                        <th class="text-center" style="width:2%;">Image</th>
                ...
                <tbody>
                    ...
                    $i = 0;
                    $file;
                    while ($row = pg_fetch_array($resultstatus)) {
                        $status = $row['status'];
                        if ($status == 'L') {
                            $class = "text-live";
